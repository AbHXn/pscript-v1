#ifndef CONDITIONALHANDLER_HPP
#define CONDITIONALHANDLER_HPP
...
pair<vector<COND_TOKENS>, vector<pair<vector<string>, size_t>>>
stringToCondTokens( const vector<string>& tokens, size_t& start, size_t& endPtr ){
	vector<pair<vector<string>, size_t>> conditions;
	
	vector<COND_TOKENS> condTokens;
	size_t bodyOpenCount = 0;

	for( ; start < tokens.size(); start++ ){
		string curToken = tokens[ start ];
		...
		if( !bodyOpenCount ){
			if( curToken == "nok" )
				...
				vector<string> condVector;

				while( start < tokens.size() ){
					if( isRegisteredCondToken( tokens[ start ] ) ){
						start--;
						break;
					}
